/*
    ...
    PURPOSE:
        Attempt simple auto-fixes on .avis files:
            • Ensure AI.FIRE.START / AI.FIRE.END exist.
        LLMs can extend this with more sophisticated fixes.
*/
...
static void autofix_avis(const char* path) {
    ...
    if (!content) {
        printf("[AUTOFIX] FAILED to read: %s\n", path);
        return;
    }
    ...
    if (modified) {
        if (write_file(path, content)) {
            printf("[AUTOFIX] %s: written with fixes\n", path);
        } else {
            printf("[AUTOFIX] %s: FAILED to write fixes\n", path);
        }
    } else {
